...
def get_sub_IDs(experiment2):
    """Collect all subject's data file names"""
...
def load_data(id):
    """read a csv given the id"""
    data = pd.read_csv(f"{RAW_DATA_DIR}/{DATA_DIR}/session-{id}.csv")
    return data


def clean_data(data):
    """remove training trials and comprehension checks"""
    data = data.loc[
        (data["trial_type"] == "horizons-trial") & (data["phase"] != "practice")
    ]
    return data


def generate_data_files():
    """converts raw csv into a nice npy data file"""
    ...
    for experiment2, fname in enumerate(["data/data_exp1.npy", "data/data_exp2.npy"]):
        ...
        good_subs = chosen_higher_mean > 0.65
        all_sub_IDs = [ID for i, ID in enumerate(all_sub_IDs) if good_subs[i]]
        all_variables = [
            variables for i, variables in enumerate(all_variables) if good_subs[i]
        ]
...
def show_comments(data):
    """display self reported strategies at the end of the experiment"""
...
def extract_variables(data):
    """convert raw data into meanigful variables of interest"""
    variables = np.zeros(
        len(data),
        dtype=[
            ("all_sd_unchosen", "<f4"),
    ...
    for trial in range(len(data)):
        ...
        # only take the free choices, thus starting at 4
        # code left choice as 0, right choice as 1
        ...
        higher_mean = np.argmax(
            [
                np.squeeze(data["r_mean_L"].iloc[trial : trial + 1]),
                np.squeeze(data["r_mean_R"].iloc[trial : trial + 1]),
            ]
        )
        higher_chosen_rate = np.sum(all_turn_choices == higher_mean) / len(
            all_turn_choices
        )
        ...
        variables["all_mean_L"][trial] = np.mean(first_four_L[forced_choices == 0])
        variables["all_mean_R"][trial] = np.mean(first_four_R[forced_choices == 1])
        ...
        variables["all_cnfind"][trial] = np.squeeze(
            data["confidence_immediate_overall"]
            .iloc[trial : trial + 1]
            .astype(int)
            .to_numpy()
        )
        ...
        if choice == 0:
            ...
            variables["all_diff"][trial] = (
                variables["all_mean_L"][trial] - variables["all_mean_R"][trial]
            )
            ...
            variables["all_diff"][trial] = (
                variables["all_mean_R"][trial] - variables["all_mean_L"][trial]
            )

        if variables["all_info_condition"][trial] == 1:
            if (variables["all_presented"][trial] < 0.5 and choice == 1) or (
                variables["all_presented"][trial] > 0.5 and choice == 0
            ):
        ...
        numbers = np.array(
            [float(num.strip()) for num in numbers_str.split(",") if num.strip()]
        )
        ...
        # legacy for when we asked about belief confidence during pilots
...
def get_choosing_higher_mean_rate(variables):
    """for excluding subjects that chose by chance"""
    V = variables
...
def select_based_on_info(variables, info, horizon):
    """Index trials with equal/unequal information or short/long horizon"""
    V = variables
...
def get_parameters_grid(n_params=3, N=20):
    """Creates an nd grid for brute search of parameter"""
    assert n_params in [2, 3, 4]
    # hard coded bounds
    X = np.linspace(-40, 40, N)  # information bias
    Y = np.linspace(0.01, 40, N)  # choice temperature
    Z = np.linspace(0, 1, N)  # lower bound of confidence scaling
    H = np.linspace(0, 1, N)  # upper bound of confidence scaling
...
def probs_to_NLL(choice_probs, variables, fit_choice_immed_total=0):
    """converts predicted probabilities into the NLL scalar"""
    V = variables
...
def get_NLL(params, variables, model_function, fit_choice_immed_total):
    """evaluates NLL of a parameter set"""
...
def eval_logistic_model(variables, params=[1.0, 1.0], fit_choice_immed_total=0):
    """runs the model on choice or confidence and returns predicted probabilities"""
    ...
    dI[V["all_presented"] > 0.5] = 1  # right shown more often, left more info
    dI[V["all_presented"] < 0.5] = -1  # left shown more often, right more info
    dQ = (V["all_mean_L"] - V["all_mean_R"] + alpha * dI) / sigma_choice
    p_L = expit(dQ)
    p_L = p_L.astype(np.double)
    p_R = 1 - p_L
    choice_probs = np.concatenate([p_L[:, np.newaxis], p_R[:, np.newaxis]], axis=1)

    right_informative_index = np.where(V["all_presented"] < 0.5)[0]
    left_informative_index = np.where(V["all_presented"] > 0.5)[0]

    if fit_choice_immed_total in [0, 1, 2]:
        choice_probs_distorted = choice_probs.copy()
        choice_probs_distorted[right_informative_index, 1] = (1 - beta) * choice_probs[
            right_informative_index, 1
        ]
        choice_probs_distorted[right_informative_index, 0] = (
            choice_probs[right_informative_index, 0]
            + beta * choice_probs[right_informative_index, 1]
        )

        choice_probs_distorted[left_informative_index, 0] = (1 - beta) * choice_probs[
            left_informative_index, 0
        ]
        choice_probs_distorted[left_informative_index, 1] = (
            choice_probs[left_informative_index, 1]
            + beta * choice_probs[left_informative_index, 0]
        )
    elif fit_choice_immed_total in [3, 4]:
...
def fit_logistic_model(variables, fit_choice_immed_total=0):
    """uses grid search to fit model parameters"""
    if fit_choice_immed_total in [0, 1, 2]:
        bounds = ((-40, 40), (0.1, 40), (0.0, 1.0))
    elif fit_choice_immed_total in [3, 4]:
        bounds = ((-40, 40), (0.1, 40), (0.0, 1.0), (0.0, 1.0), (0.0, 1.0))
    else:
        bounds = ((-40, 40), (0.1, 40))
    params = brute(
        get_NLL,
        args=(variables, eval_logistic_model, fit_choice_immed_total),
        ranges=bounds,
        Ns=50,
        finish=None,
    )
    NLL = get_NLL(params, variables, eval_logistic_model, fit_choice_immed_total)
    return params, NLL
